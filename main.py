...
async def onmessage(bot:TelegramClient,ev: NewMessage.Event,loop,ret=False):
    ...
    if username not in tl_admin_users:
        await bot.send_message(ev.chat.id,'❌ ℕ𝕠 𝕥𝕚𝕖𝕟𝕖𝕤 #𝕒𝕔𝕔𝕖𝕤𝕠, 𝕔𝕠𝕟𝕥𝕒𝕔𝕥𝕒 𝕔𝕠𝕟 @bladimirlorenzo ❌')
        return
    ...
    if '/start' in text:
        ...
        reply += '#𝔹𝕠𝕥 𝕡𝕖𝕣𝕥𝕖𝕟𝕖𝕔𝕚𝕖𝕟𝕥𝕖 𝕒 𝕝𝕒 𝕔𝕒𝕕𝕖𝕟𝕒 bladimirlorenzo\n\n'
    ...
    if '/admin' in text:
        ...
        t = '🙋‍♂️🗣 @' + username + '\n\n>> ' +  txc[1]
        print(t)
        message = await bot.send_message(838890371,txc)
    ...
    if '/up' in text:
        ...
        try:
            ...
            await bot.send_file(ev.chat,txtsendname,
                                caption=f'{txtinfo}',
                                thumb='thumb.png',
                                buttons=[Button.url('bladimirlorenzo','https://t.me/bladimirlorenzo')])
            await bot.send_file(ev.chat,premium)
            #await bot.send_file('-1001724650213',txtsendname,
                                #caption=f'{txtinfo}',
                                #thumb='thumb.png',
